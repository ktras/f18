...
public:
  ExternalVisitor(std::ostream &out, preStatementType *preStatement)
    : out_{out}, preStatement_{preStatement} {}
  ...
  void Build(const std::string &x) { Put(x); }
  void Build(int x) { Put(std::to_string(x)); }
  void Build(unsigned int x) { Put(std::to_string(x)); }
  void Build(long x) { Put(std::to_string(x)); }
  void Build(unsigned long x) { Put(std::to_string(x)); }
  void Build(long long x) { Put(std::to_string(x)); }
  void Build(unsigned long long x) { Put(std::to_string(x)); }
  void Build(char x) { Put(x); }
  ...
  void Build(const Program &x) {  // R501
...
void ExternalVisitor::Put(char ch) {
  int sav = indent_;
  if (openmpDirective_) {
    indent_ = 0;
  }
...
void ExternalVisitor::Put(const char *str) {
  for (; *str != '\0'; ++str) {
    Put(*str);
  }
}

void ExternalVisitor::Put(const std::string &str) {
  for (char ch : str) {
    Put(ch);
  }
}

void ExternalVisitor::PutKeywordLetter(char ch) {
...
void ExternalVisitor::Word(const char *str) {
  for (; *str != '\0'; ++str) {
    PutKeywordLetter(*str);
  }
}

void ExternalVisitor::Word(const std::string &str) { Word(str.c_str()); }

void ExternalVisit(std::ostream &out, const Program &program,
    preStatementType *preStatement) {
  ExternalVisitor visitor{out, preStatement};
